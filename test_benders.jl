...
inDir_str = [dir_str * "_basis",dir_str * "timeSeries/" * string(res)] # input directory

coefRngHeu_tup = (mat = (1e-3,1e5), rhs = (1e-1,1e5))
coefRngTop_tup = (mat = (1e-3,1e5), rhs = (1e-1,1e5))
coefRngSub_tup = (mat = (1e-3,1e5), rhs = (1e-1,1e5))

scaFacHeu_tup = (capa = 124, capaStSize = 124, insCapa = 1e1, dispConv = 1024, dispSt = 1e5, dispExc = 1024, dispTrd = 1024, costDisp = 1e1, costCapa = 124, obj = 1e0)
scaFacTop_tup = (capa = 1e0, capaStSize = 1e1, insCapa = 1e0, dispConv = 1024, dispSt = 1e5, dispExc = 1024, dispTrd = 1024, costDisp = 1e1, costCapa = 1e0, obj = 1024)
scaFacSub_tup = (capa = 124, capaStSize = 124, insCapa = 1e1, dispConv = 1e1, dispSt = 124, dispExc = 1e1, dispTrd = 1e1, costDisp = 1e0, costCapa = 124, obj = 1e1)
...
sub_tup = tuple(collect((x,y) for x in 1:1, y in 1:scr)...)

# options for different models
optMod_dic = Dict{Symbol,NamedTuple}()

# options for model generation 
optMod_dic[:heu] =  (inputDir = inDir_str, resultDir = dir_str * "results", suffix = suffix_str, supTsLvl = 2, shortExp = 5, coefRng = coefRngHeu_tup, scaFac = scaFacHeu_tup)
optMod_dic[:top] =  (inputDir = inDir_str, resultDir = dir_str * "results", suffix = suffix_str, supTsLvl = 2, shortExp = 5, coefRng = coefRngTop_tup, scaFac = scaFacTop_tup)
optMod_dic[:sub] =  (inputDir = inDir_str, resultDir = dir_str * "results", suffix = suffix_str, supTsLvl = 2, shortExp = 5, coefRng = coefRngSub_tup, scaFac = scaFacSub_tup)
...
top_m.parts.bal.cns[:capaBal][1,:cns]

top_m.parts.tech[:methanePyrolysis].par[:desFac].data
sub_dic[(1,1)].parts.tech[:methanePyrolysis].par[:desFac].data

printIIS(sub_dic[(1,1)])

printObject(sub_dic[(1,1)].parts.bal.cns[:enBalProcessHeat_high],sub_dic[(1,1)])